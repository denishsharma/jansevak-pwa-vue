<template>
    <BottomSheet id="select-feedback-type" ref="refBottomSheet">
        <div class="pb-4 pt-3 flex flex-col h-full overflow-y-auto">
            ...
            <PageHeading class="mb-3 shrink-0" emphasis="Type" title="Feedback">
                <template #subtitle>
                    Select the type of feedback you want to give. You can always change this later.
                </template>
            </PageHeading>

            <AppComponentBase class="overflow-y-auto">
                <div class="flex flex-wrap gap-1.5 py-2 overflow-y-auto" data-ref="parent">
                    <div v-for="(type, index) in types" :class="selectedType?.id === type.id ? 'border-orange-500 text-orange-500 bg-orange-100' : 'border-gray-200 hover:bg-gray-100 active:bg-gray-200'" :data-index="index" :data-selected="selectedType?.id === type.id" class="py-2.5 px-3 w-full font-medium text-xs select-none inline-flex text-left items-center gap-2 rounded-lg border focus:outline-none transition-all" data-visible="" @click="setSelectedType(type.id)">
                        {{ type.name }}
                    </div>
                </div>
            </AppComponentBase>
...
const selectedType = ref<{ id: string, name: string } | null>(null);
...
const setSelectedType = (id: string) => {
    ...
    if (type) {
        ...
        emit("on-type-selected", type);
        router.back();
    }
};

const emit = defineEmits(["on-type-selected"]);
