<template>
    <BottomSheet id="select-ward" ref="refBottomSheet" :do-on-close="doOnClose" :do-on-open="doOnOpen" class="max-h-[98%]">
        <div class="pb-4 pt-3 flex flex-col h-full overflow-y-auto">
            ...
            <PageHeading class="mb-3 shrink-0" emphasis="Ward" title="Select">
                <template #subtitle>
                    Select your ward for Jansevak to serve you better. You cannot change this later.
                </template>
            </PageHeading>
            ...
            <AppComponentBase class="mt-4 shrink-0">
                <label class="relative grow">
                    <input :class="searchTerm.length > 0 && 'pr-10 border-orange-300'" :value="searchTerm" class="py-2.5 px-2 pl-10 block w-full border-gray-200/[0.7] rounded-lg text-sm focus:z-10 focus:border-orange-500 focus:ring-orange-500" name="search_query" placeholder="Search query category" type="text" @input="setSearchTerm">
...
const setSelectedWard = (code: string) => {
    if (code === selectedWard.value?.code) {
        return;
    }

    const ward = wards.value.find((ward) => ward.code === code);
    if (ward) {
        selectedWard.value = ward;
        emit("on-ward-selected", ward);
        router.back();
    }
};

const fuseOptions = computed<UseFuseOptions<WardItem>>(() => ({
    fuseOptions: {
        keys: ["name", "code"],
...
const fetchWards = () => {
    executeAfter(() => {
        const { isLoading, execute } = WardService.list(handleWardListOnSuccess);
...
const emit = defineEmits(["on-ward-selected"]);
