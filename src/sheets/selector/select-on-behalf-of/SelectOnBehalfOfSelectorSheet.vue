<template>
    <BottomSheet id="select-on-behalf-of" ref="refBottomSheet" :do-on-close="doOnClose" :do-on-open="doOnOpen">
        <div class="pb-4 pt-3 flex flex-col h-full overflow-y-auto">
            ...
            <PageHeading class="mb-3 shrink-0" emphasis="For" title="Create Query">
                <template #subtitle>
                    Select user for whom you want to create a query for. You can also search for a user.
                </template>
            </PageHeading>
                        ...
                        <div v-for="(result, index) in searchUserResults" :key="result.item.id" ref="refUserItems" :class="selectedUser?.id === result.item.id ? 'border-orange-500 text-orange-500 bg-orange-100' : 'border-gray-200 hover:bg-gray-100 active:bg-gray-200'" :data-index="index" :data-selected="selectedUser?.id === result.item.id" class="py-2 px-3 w-full font-medium text-xs select-none inline-flex text-left items-center gap-2 rounded-lg border focus:outline-none transition-all" data-visible="" @click="setSelectedUser(result.item.id)">
                            <div class="flex items-center gap-x-2 w-full">
                                ...
                                <div class="flex gap-1 shrink-0">
                                    <div v-for="(val, index) in result.item.from.split(',')" :key="index" :class="selectedUser?.id === result.item.id ? 'bg-orange-200' : 'bg-gray-100 text-gray-500'" class="uppercase text-xxs font-semibold px-2 rounded py-0.5 max-w-[5rem] overflow-hidden whitespace-nowrap text-ellipsis">
                                        {{ val }}
                                    </div>
                                </div>
                            </div>
            ...
            <AppComponentBase class="mt-4 shrink-0">
                <label class="relative grow">
                    <input :class="searchTerm.length > 0 && 'pr-10 border-orange-300'" :value="searchTerm" class="py-2.5 px-2 pl-10 block w-full border-gray-200/[0.7] rounded-lg text-sm focus:z-10 focus:border-orange-500 focus:ring-orange-500" name="search_query" placeholder="Search user" type="text" @input="setSearchTerm">
...
const handleOnBehalfOfListOnSuccess = (data: any) => {
    console.log(data);
    ...
    _users.push({
        id: data.self.uuid,
        full_name: data.self.profile.full_name,
        avatar_url: resolveFileUrl(data.self.profile.avatar_url) || "",
        user_type: data.self.user_type,
        type: "self",
        from: "Self",
    });
    ...
    if (data.family && data.family.length > 0) {
        data.family.forEach((familyMember: any) => {
            _users.push({
                id: familyMember.uuid,
                full_name: familyMember.profile.full_name,
                avatar_url: resolveFileUrl(familyMember.profile.avatar_url) || "",
                user_type: familyMember.user_type,
                type: "family",
                from: "Family",
            });
        });
            ...
            if (group.users && group.users.length > 0) {
                group.users.forEach((member: any) => {
                    _users.push({
                        ...
                        avatar_url: resolveFileUrl(member.profile.avatar_url) || "",
    ...
    // merge same user
    const _usersMap = new Map<string, UserItem>();
    _users.forEach((user) => {
        if (_usersMap.has(user.id)) {
            const _user = _usersMap.get(user.id);
            if (_user) {
                const _from = _user.from.split(",").map(t => t.trim());
                if (_user.type === "group" && user.type === "group") {
                    const [s] = _from.splice(-1);
                    let count = parseInt(s.split(" ")[0]) || 1;
                    _from.push(++count + " group");
                } else if (_user.type === "family" && user.type === "group") {
                    _from.push(user.from);
                }
                _user.from = _from.join(", ");
                _user.type = user.type;
                _usersMap.set(user.id, _user);
            }
        } else {
...
const fetchUsers = async () => {
    executeAfter(() => {
        const { isLoading, execute } = QueryService.listOnBehalfOf(handleOnBehalfOfListOnSuccess);
