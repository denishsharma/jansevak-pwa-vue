    ...
    <div class="grow -mb-8 pb-8">
        ...
        <AppComponentBase>
            <transition mode="out-in" name="fade">
                <AnnouncementListSkeleton v-if="isFetching" key="skeleton" />
                <div v-else class="flex flex-col gap-4">
                    <div v-for="(announcement, index) in announcements" :key="index" class="bg-white select-none flex flex-col divide-y divide-gray-200 overflow-y-hidden rounded-lg border border-gray-200 shadow-[0_2px_6px_0_rgba(0,0,0,0.02)]">
                        <div class="px-3.5 py-2 flex flex-col gap-2">
                            <div class="flex items-center justify-between gap-4">
                                ...
                                <div class="text-gray-400 h-3 flex items-center gap-1 font-medium text-xxs">
                                    <div>
                                        <UseTimeAgo v-slot="{ timeAgo }" :time="DateTime.fromISO(announcement.published_at).toJSDate()" :update-interval="1">
                            ...
                            <div class="flex flex-col gap-1.5 my-0.5">
                                <div v-if="announcement.cover_url" class="aspect-video bg-gray-100 rounded-lg mb-1 overflow-hidden">
                                    <img :src="announcement.cover_url" alt="" class="object-cover object-center w-full h-full pointer-events-none" draggable="false" />
                                </div>
                        ...
                        <button class="px-3.5 py-2 text-xs gap-2 inline-flex text-gray-500 items-center font-medium text-left hover:bg-gray-50 active:bg-gray-100 focus:outline-none transition" @click="openAnnouncementDetailModalPage(announcement.slug)">
...
import AnnouncementListSkeleton from "@/skeleton/AnnouncementListSkeleton.vue";
...
const openAnnouncementDetailModalPage = (slug: string) => {
    executeAfter(() => {
        refAnnouncementDetailModalPage.value?.openModal(slug);
    });
};

const handleAnnouncementFetchOnSuccess = (data: any) => {
    console.log(data);
    announcements.value = data.map((item: any) => {
        return {
            ...
            published_at_formatted: useDateFormat(item.published_at, "DD MMM, YYYY - h:mm A"),
            cover_url: resolveFileUrl(item.cover_url) || "",
        };
    });
};

const handleAnnouncementFetchOnFailure = (error: any) => {
    console.log(error);
};
...
const fetchAnnouncements = () => {
    executeAfter(() => {
        const {
            isLoading,
            execute,
        } = AnnouncementService.list(undefined, handleAnnouncementFetchOnSuccess, handleAnnouncementFetchOnFailure);
