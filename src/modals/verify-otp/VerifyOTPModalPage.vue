        ...
        <Dialog as="div" class="relative z-40">
            ...
            <div class="fixed inset-0 overflow-hidden">
                <div class="flex h-full">
                    ...
                    >
                        <DialogPanel class="w-full h-full bg-white transform overflow-hidden transition-all">
                            <AppContainerBase>
                                <template #title>Verify OTP</template>

                                <template #header>
                                    <div class="flex items-center">
                                        <button @click="closeModal" type="button" class="-ml-[0.725rem] py-2 px-2 select-none inline-flex flex-shrink-0 items-center gap-2 rounded-full border border-transparent text-gray-800 hover:bg-gray-100 focus:outline-none transition-all">
                                ...
                                <template #body>
                                    ...
                                    <AppComponentBase class="mt-3.5">
                                        <div class="flex flex-col gap-2">
                                            <button @click="verifyOtp" type="button" class="py-3.5 px-4 w-full font-medium text-xs select-none inline-flex justify-center items-center gap-2 rounded-lg border border-transparent  transition-all" :class="clsx({'bg-orange-500 text-white hover:bg-orange-600 active:bg-orange-700 focus:outline-none focus:ring-2 focus:ring-orange-500 focus:ring-offset-2': !isVerifying, 'bg-gray-100 text-gray-300': isVerifying})" :disabled="isVerifying">
                                                <div v-show="isVerifying" class="animate-spin inline-block w-4 h-4 border-[3px] border-current border-t-transparent text-gray-300 rounded-full" role="status" aria-label="loading"></div>
